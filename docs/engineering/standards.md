...
| **AI** | Any endpoint speaking OpenAI's Chat Completions API, selected by `LLM_BASE_URL` / `LLM_API_KEY` / `LLM_MODEL`; default is OpenAI direct on `gpt-4o-mini` (`services/nlp-worker/src/nora_nlp/settings.py`) | Strict JSON Schema, low temperature, logs without PII. ADR 0004. Azure OpenAI is one compatible endpoint among several, **not a tier**: the Azure *managed services* left with ADR 0034, and the substrate turned out to be an Azure VM after all (ADR 0051) — neither fact makes Azure OpenAI special here. The control plane's per-service binding (ADR 0024) is the runtime override, and the worker does not read it |
...
| **Infra** | Self-hosted: single Azure Ubuntu VM, Docker Compose (ADR 0034/0051) | Cloudflare Tunnel ingress, SOPS + age secrets, pull-based deploy |
...
**Each cell carries the date it was measured on**, because the surfaces are re-measured by different
runs and a single date over the whole table is a date that is wrong for most of it. That was the
state on 2026-08-23: the table said "Measured 2026-08-17" and the web row had been left behind by
two waves of work.

| Area | Sustained target (ADR 0018) | Measured |
|---|---|---|
| **Critical areas** (IAM, Auth, PII, LLM analyzer) | **> 85%** | **2026-08-17** — IAM packages 90.9% instr · Auth/identity packages 93.8% instr · PII shield 96.6% stmt · `llm_analyzer.py` 84.7% stmt |
| Other backend areas | > 60% | **2026-08-17** — overall backend 79.5% instruction / 79.9% line (see the repeatability note below). The suite has since grown from 713 to **809 `@Test` methods**, so this figure is stale in an unknown direction |
| NLP Worker | > 85% | **2026-08-23** — **94%** statement over `nora_nlp` (1,682 statements, 97 uncovered) across **1,194 tests** |
| **Backend branch coverage** | > 70% | **2026-08-17** — **61.5-61.6%**, short of the target by about 8.5 points. A bundle floor of 55% branch now gates it, so it cannot slide further while it climbs |
| Web Next.js | ADR 0018's per-page table (auth pages > 50%, dashboard/meeting-detail/tasks > 40%, shared components > 60%) — **not enforced, not met** | **2026-08-23** — **26.42%** statement whole-app (20.68 branch / 20.46 functions / 26.75 line) over **319 tests in 25 files**, up from 9.37% over 150 tests when the first screen tests landed. This cell was re-measured twice on the same day and moved 6.5 points between the two readings, because a second wave of work landed in between — which is the argument for reading the number out of a run rather than out of this table. Gated module figures are not reproduced here: the config is what decides, and four documents once carried four different lists |
| Operator console (`apps/admin`) | none in ADR 0018 — the console did not exist | **2026-08-23** — **26.93%** statement whole-app over **64 tests in 4 files**, with `access.ts` at 100/96.87/100/100 and gated there alone. Before this date the console had **no test of any kind**, including over the module that decides whether it is fail-open or fail-closed |
| Desktop client | out of scope in ADR 0018 | Not measured as a percentage. It has **38 Rust tests** (`cargo test`) and **13 TypeScript tests** on Node's own runner since 2026-08-23; before that it had 25 Rust tests and nothing on the TypeScript side |

**Where these come from.** `scripts/report-coverage.sh` runs in the `api`, `worker` and `web` CI jobs and prints the figures to the job log and to the run summary page. It reads the report the test run just wrote (`target/site/jacoco/jacoco.csv`, `.coverage`, `apps/web/coverage/coverage-summary.json`) rather than measuring anything itself, so the same command on a workstation gives the same number. **The `admin` job is the exception and is called out rather than glossed:** it runs `npm run test:coverage`, which applies the threshold and prints v8's own table, but `report-coverage.sh` has no `admin` scope, so the console's figure never reaches the run summary beside the other three. That is a one-function gap in the script, not a missing measurement. Read the current figures in the run; the date in each cell is when that cell was last copied from one, not a promise about today.

**Some of these are gates. The rest are reports, and the list of gates lives in the build files, not here.** `mvn verify` fails on three JaCoCo rules (the class `PolicyEvaluator`, the package `domain.iam`, and the whole bundle — `services/api/pom.xml`); the worker job fails on `--cov-fail-under=90` over `pii_shield`; the web and admin jobs fail on the per-module `coverage.thresholds` in their `vitest.config.mts`. **This paragraph deliberately no longer enumerates the gated modules.** It listed five when the config declared six, `AGENTS.md` listed a different five, `README.md` a fourth set and `apps/web/README.md` a fifth — four documents, four answers, none matching the file that decides. Read the config. Nothing fails on any other row, including the branch-coverage row that misses its target and the whole-app web row. The front-end numbers are **floors deliberately set below the measured rate** (ADR 0042): they exist to catch a regression, not to certify a level, and raising coverage means raising the floor in the same pull request.

**The overall backend figure is not repeatable, and that is measured, not assumed.** Three CI runs of the same branch reported 77.1%, 77.2% and 77.3% instruction (32,083 / 32,128 / 32,138 covered of a constant 41,596), with branch at 61.5-61.6% and line at 78.0-78.1%. **A second thing about that denominator, recorded 2026-08-23 because it is unresolved rather than because it is understood:** running `mvn test jacoco:report` on a workstation over the same tree reports **53,500** total instructions, not 41,596. Neither the JaCoCo configuration nor `report-coverage.sh` excludes a single row, so the gap is not filtering and no explanation here would be more than a guess. Consequence: quote the *ratio*, never the absolute, and compare absolutes only between runs of the same command on the same machine — which is how the bundle floor was re-checked after the 2026-08-23 audit round (53,500 → 55,058, +2.9%, leaving the 0.70 floor with room even if none of the new code were covered). Something in the integration suite takes a slightly different path from run to run. The per-area rows above did **not** move: IAM, Auth and `PolicyEvaluator` came out byte-identical all three times, which is the useful half of this finding — the numbers ADR 0018 actually cares about are stable, and the aggregate is the noisy one. Consequence for anyone quoting these: a backend total is good to about ±0.1 point, so a decision that turns on a tenth of a percent is a decision resting on nothing.
...
| Date | Change |
|---|---|
| 2026-08-23 | **§9's coverage table was re-measured where it could be, and dated per cell where it could not.** This document is named by `README.md` and `docs/product/roadmap.md` as the single source for coverage, and its web row had been left behind by two waves of work — it said 9.54% over five gated modules while `vitest.config.mts` declared six and the suite measured 19.88% over 263 tests. The web and admin figures here were taken by running both suites. The backend and worker rows were first left at their 2026-08-17 date, because the machine running the audit had neither Maven nor Python — and then both were installed and the rows that could be re-measured were: the **worker** row is now 2026-08-23 (94% over 1,194 tests, and the PII shield at 97%), and the **backend** row keeps its date because its coverage comes from Testcontainers, which needs a Docker daemon that is still absent. A re-dated figure that was not re-measured is the exact failure this section documents from May, so the two cases are marked differently rather than uniformly. New rows for `apps/admin` (which had no test of any kind before this date) and for the desktop's Rust and TypeScript suites. The paragraph that enumerated the gated modules **stopped enumerating them**: four documents listed four different sets, none matching the config that decides. Also recorded here rather than left to be rediscovered: `scripts/report-coverage.sh` has no `admin` scope, so the console's coverage is applied as a gate but never printed beside the other three. |
