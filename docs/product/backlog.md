...
> auditable. The tree has moved since: as of 2026-08-23 it carries **32 migrations** (last
> `V033__create_iam_permission_boundaries.sql`, with `V031` a permanent gap — see
> `standards.md` §Migrations), **21 controllers** with **104 mapped handlers**, and **50 numbered
> ADRs, 45 of them with status `accepted`** — three are superseded outright (0009, 0014, 0035) and
> two partially (0016, 0026), whose surviving halves still bind. That last figure read "49 numbered
> ADRs, 36 of them accepted" until 2026-08-23 and no counting rule produced the 36: it was the number
> compatible with the anchor's universe of 41 ADRs, and it was the one figure in this paragraph
> nobody had recounted — in the paragraph whose whole purpose is to prove the figures were
> recounted. `docs/adr/README.md` now carries the split with the command that reproduces it.
>
> **Evidence cites a symbol, not a line number, since 2026-08-23.** Every `Controller.java:NNN` in
> the column below had drifted — `MeetingsController.upload` was cited at 175 and sat at 268, and
> the IAM rows were off by roughly twenty — so the document's own audit mechanism ("open the file it
> cites") landed the reader on the wrong endpoint precisely in the files that changed most. Line
> numbers were not re-anchored, because re-anchoring buys one commit of accuracy: they are replaced
> by the method or constant name, which `grep` resolves and a refactor moves along with the code.
> `scripts/check-doc-links.sh` can only verify that a cited line exists, never that it still means
> what the prose says — which is why the drift ran for weeks with a green gate.
>
...
|---|---|---|---|---|---|
| US01 | Create an account with e-mail and password | M | DONE | `AuthController.signup` · PR #4 | — |
| US02 | Verification e-mail after signup | M | DONE | `AuthController.verifyEmail` · migration V003 · PR #4 | E-mail delivery depends on the Resend/log adapter (configurable per env) |
| US03 | E-mail/password login | M | DONE | `AuthController.login`, with the session kept alive by `refresh` and ended by `logout`, and the caller's own identity read from `me` (`GET /auth/me`) · PRs #4 + #59 | — |
| US04 | Reset password via link | M | DONE | `AuthController.requestPasswordReset` + `confirmPasswordReset` · PRs #4, #47 | — |
...
| US06 | Invitation to the tenant by corporate e-mail | M | DONE | `InvitationController.invite` + `accept`, with `list` (`GET /iam/invites`, `iam:invite:read`) and `revoke` (`DELETE /iam/invites/{id}`, `iam:invite:revoke`) closing the loop · migration V010 · V018 (token stored as SHA-256) · ADR 0011 · PR #55 | — |
| US52 | Change my own password from inside the app | S | DONE | `POST /auth/password/change` (`AuthController.changePassword`) · security section of the settings hub in `apps/web/src/app/(app)/settings/context/page.tsx` · PR #231 | — |
| US53 | Sign out of every device at once | S | DONE | `POST /auth/logout-all` (`AuthController.logoutAll`) · revokes the whole refresh-token family (ADR 0020, migration V014) | — |
| US54 | Edit or delete my own account | S | DONE | `PATCH /users/me` (`UsersController.updateMe`) and `DELETE /users/me` (`deleteMe` → `AuthService.deleteAccount`) · `SettingsFlowIntegrationTest` · PR #231 | **Deletion is a physical erasure of the whole workspace, and it is bounded rather than soft.** `deleteAccount` refuses with `AccountNotPersonal` unless the tenant has exactly one member, then hard-deletes the tenant and lets the CASCADE carry meetings, transcripts, analyses, chat and flows — total forgetting is the goal (ADR 0029), so no audit row survives either. **This row called it "the soft-delete of ADR 0021" until 2026-08-23 and that was simply wrong** — the bound and the CASCADE were always there, the description was not, and a reader who trusted it would have read the method as a bug. It is still not general tenant deletion, which is US80 and is a declared deferral |
| US55 | Ask for the verification e-mail again | C | DONE | `POST /auth/verify-email/resend` (`AuthController.resendVerification`) | — |

### E2 — Meeting Ingestion

| ID | Title | MoSCoW | Status | Evidence | Known debt |
|---|---|---|---|---|---|
| US07 | Transcript upload (`.txt`, `.vtt`, `.srt`) | M | DONE | `MeetingsController.upload` · migration V004 · PR #5 | The 10 MB per-file ceiling is enforced on both sides since 2026-08-23 — `readFile` on the server, `spring.servlet.multipart` above it, and the file picker in the browser, which used to advertise the limit and never check it |
| US08 | Audio/video upload (`.mp3`, `.mp4`) | **W** | **WONT** | `MeetingsController.ALLOWED_FORMATS = {TXT,VTT,SRT}` and `ALLOWED_EXTENSIONS` | **WONT by ADR 0046 §2**, and the reason is now verifiable where the inherited one was not. ADR 0014 gated this on ">30% of uploads are audio in a pilot", a criterion with no pilot to measure. The real reason: transcription is a **streaming realtime** session since ADR 0039/0045, so a file upload needs the provider's **batch** API — a second provider surface, credential path, cost model and failure mode. It is a new capability wearing the clothes of a small addition |
| US09 | Live capture on the Desktop | **W** (declared W in the backlog, but **implemented**) | DONE | `apps/desktop/src-tauri/src/system_audio.rs` (WASAPI loopback), `audio_capture.rs`, `audio_resample.rs`, `stt.rs`, `live_analysis.rs` · `POST /meetings/live-analyze` (`MeetingsController.liveAnalyze`) · PRs #8, #65 · ADR 0008 | **Windows-only** — `system_audio.rs` raises `compile_error!` on any other target, and the macOS (BlackHole) and Linux (PulseAudio) paths were deleted by ADR 0038 §2, never having been exercised. **The transcription engine was replaced**: ADR 0039 (contract in ADR 0045) moved STT to OpenAI's realtime API over an ephemeral session token minted by `POST /stt/sessions`; `stt_cloud.rs` and `stt_token.rs` are the client, and `stt_local.rs` / `whisper_model.rs` are out of the tree. The audio now leaves the machine unredacted — ADR 0040 is where that is recorded. Validation in a real Windows/Teams environment is still pending |
| US10 | Name and categorize the meeting at upload | S | DONE | `MeetingUploadMetadata` accepts `title` and `tags` · consumed by `MeetingsController.upload` | Since 2026-08-23 the metadata is validated rather than merely parsed: bean constraints on every field, `@Valid` cascading into `participants`, and a ceiling on the number of tags |
...
| US57 | Split one file that contains several meetings | S | DONE | `POST /meetings/split-preview` (`MeetingsController.splitPreview`) · worker `/split` endpoint · `SplitPreviewIntegrationTest` · PRs #254, #255 | Preview-then-confirm; the segmentation is heuristic and the user accepts or rejects the cut. Since 2026-08-23 the call is rate-limited and its provider cost is reported, which it was not: `/split` calls the provider once per window, so a large file was the cheapest way to spend money unobserved |
...
| US12 | Extracted tasks and decisions | M | DONE | `actionItems` + `decisions` in `MeetingAnalysisV1` · `GET /tasks` (`TasksController.list`) | — |
...
| US18 | Search by keyword/period | M | DONE | `MeetingsController.list` accepts `search`, `status`, `from` and `to`, assembled by `buildFilter` | — |
...
| US61 | Projects view: meetings grouped by tag | C | DONE | `apps/web/src/app/(app)/projects/page.tsx` · `build-projects.test.ts`, `paged.test.ts` · PR #165 | Client-side grouping over `MeetingListItem.tags`, with drill-down through `?tag=`. **There is no project entity and no backend**: a "project" is a tag, so renaming or merging one is not possible. Three things it got wrong until 2026-08-23: it was built on the first 100 meetings with no paging and no notice that it had truncated; it labelled a raw action-item total as "open" items; and an unknown `?tag=` fell back to the whole listing instead of saying the tag does not exist |

### E5 — Task Management

| ID | Title | MoSCoW | Status | Evidence | Known debt |
|---|---|---|---|---|---|
| US22 | Consolidated task list | M | DONE | `TasksController.list` · `apps/web/src/app/(app)/tasks/page.tsx` | The endpoint **paginates** since 2026-08-23. It used to load every action item of the tenant with no `LIMIT` and no page parameter, on both IAM paths — the same two paths `GET /meetings` had already learned to bound |
| US23 | Mark a task as completed | M | DONE | `PATCH /tasks/{id}` accepts `status` (`TasksController.update`) | — |
...
| ID | Title | MoSCoW | Status | Evidence | Known debt |
|---|---|---|---|---|---|
| US35 | Create IAM groups | M | DONE | `IamController.createGroup`, `listGroups`, `deleteGroup` · migration V006 · PR #35 | — |
| US36 | Create and version JSON policies | M | DONE | `IamController.createPolicy` / `updatePolicy` / `getPolicy` / `listPolicies` / `deletePolicy` · `iam_policy_versions` table in V006, **readable since 2026-08-23** through `listPolicyVersions` (`GET /iam/policies/{id}/versions`, `iam:policy:read`) | Conditions: `StringEquals`/`StringIn`/`StringLike`/`DateGreaterThan`/`DateLessThan`. The version table was write-only from V006 until 2026-08-23 — every update appended a row and no endpoint, screen or query ever read one back, so the "immutable history" the acceptance criteria of §7 promise could not be inspected by the tenant it belonged to |
| US37 | Attach/detach policies to groups and users | M | DONE | `IamController.attachToGroup` / `detachFromGroup` / `attachToUser` / `detachFromUser` · composite FK `(tenant_id, user_id)` on `iam_user_policies` since V027 | — |
| US38 | Add/remove users from groups | M | DONE | `IamController.addMember` / `removeMember` / `listGroupMembers` · composite FK on `iam_user_groups` since V027 | `listGroupMembers` had existed since V006 with no caller; the IAM screen lists members since 2026-08-23, so a group's contents are no longer edited blind |
...
| US40 | IAM audit log | M | DONE | `IamController.listAudit` · `iam_audit_events` table in V006 | The auth audit log (login/refresh/logout) exists separately since PR #118; there is still no single global `audit_events` table |
...
| US51 | Alert when Account Health changes band | **W** | **WONT** | No code | **Closed by ADR 0038 §4.** It alerts on the band computed by US50 and cannot outlive it. UC30 of `docs/challenge/use-case-diagram.md` described the alert firing until 2026-08-23; that use case now carries the WONT marker and the reason, and is kept rather than deleted so the original modelling stays legible |

### E11 — Conversational Assistant

| ID | Title | MoSCoW | Status | Evidence | Known debt |
|---|---|---|---|---|---|
| US67 | Ask NORA about my workspace in a chat | S | DONE | `apps/web/src/app/api/chat/route.ts` — streaming BFF, runs entirely server-side so the LLM key never reaches the browser · `apps/web/src/app/(app)/chat/page.tsx` · ADR 0004 (provider-agnostic) · PR #165 | Input is budgeted rather than unbounded: the last 16 messages, 8k characters per message and 24k across the retained history. Since 2026-08-23 the screen calls the route through `streamChat` in `client.ts` rather than a bare `fetch`, so an expired session is refreshed instead of rendering a permanent error bubble, and the stream carries a 45-second **idle** budget — not a total one, which would cut a long healthy answer |
| US68 | Chat answers grounded in my own meetings | S | DONE | The route calls `GET /meetings/search?q=&k=6` (`fetchContextMeetings` in `route.ts`), which is US15's embedding search, and falls back to the most recent meetings when the search returns nothing · PR #206 | Inherits US15's gap: meetings analysed before PR #206 have no embedding and can only be reached by the recency fallback — US86's backfill is the way out |
| US69 | Chat answers use the company context | S | DONE | The route fetches `GET /tenant/context` (`fetchTenantContext` in `route.ts`) and injects products, ICP, glossary and competitors into the system prompt · PR #467 | **This was the product's central claim and the chat did not do it until PR #467.** The analysis path (US14) had been reading the same context since V005 |
| US70 | Persistent chat sessions I can revisit, rename and delete | S | DONE | `ChatSessionController.list` / `create` / `get` / `appendMessage` / `rename` / `delete` · migration V022 (`chat_session`, `chat_message`, both with RLS `tenant_isolation`) · sidebar `apps/web/src/components/core/app-sidebar-sessions.tsx` · `ChatSessionIsolationIntegrationTest` · PR #219 | Sessions are scoped by tenant **and** by user: a user never sees another user's sessions, not even Root |

### E12 — Automation (NORA Flows)

| ID | Title | MoSCoW | Status | Evidence | Known debt |
|---|---|---|---|---|---|
| US71 | Build an automation on a visual canvas | S | DONE | `apps/web/src/app/(app)/flows/` — `page.tsx`, `new/page.tsx`, `[id]/page.tsx`, `catalog.tsx`, `flow-editor.tsx`, `side-panel.tsx` · `flow-editor.test.tsx` · React Flow styled with NORA tokens · ADR 0032 · PRs #223, #226, #248, #250 | The block that lands on an empty canvas is the first trigger in `CATALOG`, so the array order is load-bearing — flagged in the file's own comment after PR #468 added two more triggers. Leaving the editor with unsaved changes stopped discarding them in silence on 2026-08-23: `beforeunload` plus an intercepted back link, with a three-way confirmation whose "save and leave" only leaves if the save actually landed |
...
| US73 | Test a flow before activating it | C | DONE | `POST /workflows/{id}/test` (`WorkflowsController.test`), gated by its own `workflow:test` IAM action because it really executes the wired actions | A test run sends real e-mail and creates real issues against the tenant's own connections. That is deliberate and is why it is a distinct IAM action, but there is no dry-run mode |
| US74 | See a flow's execution history | S | DONE | `GET /workflows/{id}/executions` (`WorkflowsController.executions`) · `workflow_executions` in migration V023 with `status` CHECK in `RUNNING`/`SUCCESS`/`FAILED` and a `log_json` JSONB trail · `ExecutionLogBuilder.java` · `WorkflowFlowIntegrationTest` | — |
...
| ID | Title | MoSCoW | Status | Evidence | Known debt |
|---|---|---|---|---|---|
| US78 | Permanently erase a meeting | S | DONE | `PrivacyController.eraseMeeting` (`DELETE /privacy/meetings/{id}`) — physical hard delete, distinct from ADR 0021's soft delete, and gated since 2026-08-23 by its own `meeting:erase` action rather than by `meeting:update`, which is the action for editing a title; the FK cascade purges the transcript (`raw_text`, the PII), participants, tags and analyses · authorization runs on the loaded meeting's attributes so an attribute-scoped Deny still fires · ADR 0029 · `PrivacyFlowIntegrationTest` · PR #204 | — |
| US79 | Retention window that purges old meetings automatically | C | DONE | `RetentionSweeper.java` · `NORA_PRIVACY_RETENTION_DAYS` · ADR 0029 · PR #204, semantics documented honestly in PR #468 | **`0` and any negative value mean retention is OFF, and that is the shipped default** — the purge is an irreversible hard delete with CASCADE, and no environment should start deleting because a variable went unset. There is no value meaning "purge immediately"; the smallest window is one day. The window is **global**: one number for every tenant, with no per-tenant column and no link to tenant status. **The landing page claimed a 30-day audio retention default and a configurable TTL, and no longer does**: issue #456 was decided on 2026-08-23 (ADR 0050) and the page now describes what `RetentionSweeper` does — purge off by default, and audio that never reaches the server at all |
| US88 | Remove a meeting from the workspace without destroying it | S | DONE | `MeetingsController.delete` (`DELETE /meetings/{id}`), gated by `meeting:delete` and authorized in-transaction on the meeting's attributes · stamps `deleted_at` from V013 · `MeetingSoftDeleteIntegrationTest` · ADR 0021 | **New row on 2026-08-23, for the writer ADR 0021's machinery never had.** V013's column and partial unique indexes, the entity's `@SQLDelete`/`@SQLRestriction` and the `deleted_at IS NULL` predicates existed since PR #114 and no code path ever set the column, so the product offered only the irreversible erasure of US78: a user who uploaded the wrong file chose between living with it and destroying PII belonging to everyone else in the meeting. Reversal itself is not exposed — the row survives and the state is recoverable in the database, but no endpoint undeletes |
...
| ID | Title | MoSCoW | Status | Evidence | Known debt |
|---|---|---|---|---|---|
| US81 | Manage the AI model catalog without a deploy | S | DONE | `PlatformAdminController.listModels` / `createModel` / `deleteModel` / `listConfig` / `bind` · separate platform datasource, ADR 0022 · ADR 0024 · `apps/admin/src/app/models/page.tsx` · PR #172 | The console's "add model" form never sent `baseUrl`, which `createModel` requires, so **every** creation through the UI failed on a backend 400 — and under `NORA_ADMIN_USE_MOCKS` it appeared to succeed. Fixed on all three sides on 2026-08-23: the field exists in the form, the input type stopped making it optional, and `createModel` validates the four required fields before spending a round trip |
| US82 | Services resolve their model at runtime | S | DONE | `PlatformInternalController.llmConfig` (`GET /internal/platform/llm-config?service={chat\|analysis\|multimodal}`) · consumed by `apps/web/src/app/api/chat/route.ts` · ADR 0024 | **Soft fallback by design**: if resolution fails the caller uses the `LLM_*` environment instead of failing, so the chat never goes down because the control plane is unreachable. The cost is that an operator's model switch can silently not apply |
| US83 | See what the AI is costing, per service and per tenant | S | DONE | `PlatformInternalController.usage` (fire-and-forget, always 202) · `PlatformAdminController.cost` (`GET /admin/platform/telemetry/cost`) · `apps/admin/src/app/telemetry/page.tsx` · ADR 0024 · PR #172 | The console screen gained the window and the `groupBy` the endpoint always accepted on 2026-08-23 — until then it rendered one hard-coded period with no per-tenant breakdown, which is half the story's title. Two provider calls that spent money without reporting it, `/split` and `/analyze-live`, emit cost telemetry from the same date. Cost is computed from the token counts providers report, not from an invoice. ADR 0039 notes that once STT moves to an ephemeral session token the audio path is not measured at all — sessions issued and minutes estimated, not bytes counted |
...
| US85 | Platform health and business telemetry | C | DONE | `PlatformAdminController.health`, `business` and `listFlags` | Platform-wide, for the operator. The flags screen says out loud that it is read-only and why — there is no write endpoint, by ADR 0022 — instead of presenting toggles that do nothing. The tenant-facing equivalent is US33 — `GET /usage` — which is a different endpoint on a different plane, not this one reused: it takes the tenant JWT and binds one tenant in the WHERE clause |
| US86 | Reindex meetings the RAG index never got | S | DONE | `PlatformAdminController.embeddingBackfillPreview` / `embeddingBackfillRun` · `EmbeddingBackfillService.java` · platform migration `V002` (retires the dead `service.search-embeddings` flag) · `EmbeddingBackfillIntegrationTest` · ADR 0044 | Indexing at the end of an analysis is best-effort, so a missing credential or a failing provider left a meeting out of `meeting_embeddings` forever — the only remedy was a full reprocess, paying for a whole LLM analysis to obtain one vector. The backfill reindexes from the summary already stored, and the same query covers a change of embedding model. **Operator-triggered on purpose**: it is billed per meeting, so there is no startup catch-up and no scheduled sweep, and a run is one tenant at a time with a ceiling. No console UI yet — it is `curl` behind Cloudflare Access |
...
| Live analysis endpoint (Desktop overlay highlights) | #65 | ADR 0039 | DONE (`MeetingsController.liveAnalyze`) |
| TF-IDF baseline package (`packages/nlp-baseline/`) | #54 | ADR 0010 | DONE. **Four modules** beside `__init__.py` — `normalize`, `tokenize`, `tfidf`, `stopwords` — and 52 tests (13 + 25 + 14) over three test files. This row said "3 modules" from the day it was written; the fourth is the stopword list, which is a module whether or not one counts it as pipeline |
| Expanded synthetic dataset | #54, #488 | — | DONE (`data/synthetic/`, `data/samples/`). #54 delivered 12 `.txt` + 3 `.vtt` + 2 `.srt` + 3 JSON contexts; **the tree now carries 17 `.txt`, 3 `.vtt`, 2 `.srt` and 4 JSON contexts**, because #488 added the `meridian-erp` tenant and meetings 13-17 for the demonstration seed. Since 2026-08-23 every file here is exercised by `services/nlp-worker/tests/test_pii_over_transcripts.py`, which asserts that a speaker whose name the shield knows disappears from the redacted text — the policy in `data/samples/README.md` was declared and untested until then |
| DS Sprint 1+2 notebook (`notebooks/01-tf-idf-eda-meetings.ipynb`) | #54 | — | DONE (26 cells: 17 code, 9 markdown) |
...
| Meeting reprocessing | #46 | — | DONE (`MeetingsController.reprocess`) |
| Soft-delete (`deleted_at` + `@SQLRestriction` + partial UNIQUEs) | #114 | ADR 0021 | DONE (V013), and **only wired to a writer on 2026-08-23** — see US88. For three months the column, the partial indexes and the `@SQLRestriction` defended a state no code path could produce |
| JWT RS256 + JWKS endpoint (`/.well-known/jwks.json`) | #117 | — | DONE (`JwksController.jwks`) |
...
| Repository normalised to English + two guard scripts | #410 | — | DONE (`scripts/check-doc-links.sh`, `scripts/check-language.sh`). Both are nets, not proofs: the language guard keys on accents plus a curated word list and never sees commit messages. `check-doc-links.sh` gained a second pass on 2026-08-23 over `` `path:line` `` citations in code spans, which is the format this document uses as evidence and which the link pass had always thrown away — it verifies the file and the line exist, never that the line still means what the prose says, which is exactly why §2 stopped citing lines |
| Audit of 2026-08-23 — the operational half | — | ADR 0050 | DONE, and recorded per story above rather than summarised here. What has no story of its own: a spend limiter in front of every path that calls the external provider (search, live-analyze, reprocess, split-preview and the MCP tool), a real JSON body ceiling — no Spring MVC property bounds a JSON body, and the three candidates were checked — server-side caps on the IAM, invite and chat listings, and the startup RLS guard learning to ask the database whether the connection is actually subject to RLS instead of trusting `NORA_RLS_ENFORCE=false`. On the infrastructure side: eight Grafana alert rules with a contact point and a notification policy, an off-host backup leg that fails loudly until its target is set, a quarterly restore drill on a timer, and `deploy.sh --follow-release`, which finally gives the published release pointer a consumer |

## 4. State Summary (2026-08-23)
...
| **Should Have (S)** | 42 | **41** | 0 | **1** (US80) | 0 |
...
- **US87 and US88 were added on 2026-08-23**, and both existed in the product before they existed
  here. US87 is reading and renaming the workspace (`GET /tenant`, `PUT /tenant/name`) — shipped,
  documented in `openapi.yaml`, and carried by no story, no MoSCoW cell and no status. US88 is the
  reversible removal of a meeting, which did not exist at all until this date and whose absence had
  left ADR 0021's whole soft-delete apparatus defending an unreachable state. Revoking an invitation
  and listing invitations were the third case and needed no new row: they belong to US06, whose
  evidence now names them. That leaves **88** stories where §2 counted 86 — the count is closed over
  the real surface again, which is the property this document claims and which the 2026-08-17
  rewrite achieved for chat, Flows and integrations and missed for these.

**Effective coverage**

- Must Have: **28 of 28** (100%). The v1.0 MVP of §6 is complete.
- Should Have: **41 of 42** (98%). **One story remains, and it is not open work**: US80,
...
| US87 — See and rename the workspace · US88 — Reversible removal of a meeting | **Recorded**, both DONE — the first was shipped without a story, the second is new | ADR 0021 (US88's mechanism) · this revision |
| The public landing page (not a user story) | **Unfrozen and rewritten against the code.** ADR 0038 DEC-04 froze it and catalogued the claims in issue #456; that freeze is lifted and the claims are gone or marked roadmap | ADR 0050 |
...
**Four of those seven were built on 2026-08-23 without the trigger firing**, which is worth stating
plainly rather than letting the ADR read as overtaken: alerting (§6a), the off-host backup leg
(§6b), the restore-drill cadence (§6c) and the roll-forward consumer (§6e) exist now. The deferral
was never "this must not be built"; it was "this is not owed to anyone, so it is not a gap" — and
building it anyway does not reopen the block. What is still deferred is deferred for reasons that
are not effort: secret rotation has no schedule, desktop code signing needs a certificate that
cannot live in a public repository (§6f), and the RLS repository default stays off (§6g).
`docs/operations/production-readiness-gaps.md` carries the gap-by-gap state.
