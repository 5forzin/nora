...
import { openAiSseToNdjson, type StreamUsage } from "@/lib/chat/sse-to-ndjson";
...
/**
 * The other half of that sentence, and the half that was missing.
 *
 * `PROVIDER_HEADER_TIMEOUT_MS` is cleared the instant the headers land, so until now nothing at
 * all bounded the BODY: a provider that flushed its headers, sent one token and then stopped held
 * the request open until the edge gave up two minutes later, with the socket to the provider live
 * for the whole window. The symptom is identical to the header stall this file already documents
 * — 504 at the edge, no `[chat] upstream done` line — which is why it survived the fix that
 * chased it.
 *
 * What this bounds is SILENCE BETWEEN CHUNKS, never total duration: a reply that is still
 * producing tokens is healthy however long it runs, and a total cap would truncate it
 * mid-sentence. 45s is far above the widest gap a streaming completion produces in practice
 * (measured under 700ms to first byte, sub-second between chunks) and far below the edge's own
 * patience, so it fires only on a stream that has actually stopped.
 */
const PROVIDER_IDLE_TIMEOUT_MS = 45_000;

/**
 * Appended to the answer when the idle budget above ends the stream. It is markdown because the
 * bubble renders markdown, and it exists so a half-written answer does not sit on screen looking
 * finished — the user has to be able to tell "the model stopped mid-thought" from "this is the
 * whole reply", and "Tentar de novo" only makes sense if they can.
 */
const PROVIDER_IDLE_NOTICE =
  "\n\n_(A resposta parou de chegar — o provedor de IA ficou em silêncio. Tente de novo.)_";
...
export async function POST(req: Request): Promise<Response> {
  ...
  phase("stream-build");
  const stream = openAiSseToNdjson(upstream.body, {
    onComplete: (usage) => {
    ...
    idleNotice: PROVIDER_IDLE_NOTICE,
  });
